    ...
    <aside className="staff-sidebar sticky top-0 flex h-screen flex-shrink-0 flex-col border-r border-slate-200 bg-white">
      ...
      <nav className="flex-1 space-y-1 px-2 py-7">
        {links.map(({ to, label, icon: Icon }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex w-full items-center gap-2 rounded-md px-3 py-2.5 text-[10px] font-semibold uppercase transition ${
                isActive
                  ? 'bg-[#005b9f] text-white shadow-sm'
      ...
      <div className="border-t border-slate-200 px-2 py-4">
        <button
          ...
          className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-xs font-semibold text-rose-700 transition hover:bg-rose-50"
        >
          <LogoutIcon />
          <span>Logout</span>
        </button>
